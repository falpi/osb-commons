...
At the end of the compilation a single "fat" (or "merged") jar archive is produced and automatically copied into the ```<DOMAIN_HOME>/config/lib``` to make the classes available in WebLogic for various purposes such as implementing Security Providers or OSB extensions (for example for extending pipelines with Java Callouts). </p>
